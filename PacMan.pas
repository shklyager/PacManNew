...
procedure ext;
begin
  assign(f1, 'records.txt'); //запись кол-ва очков в файл records.txt
  append(f1);
  write(f1, ' ', o);
  close(f1);
  window.Close;
...
begin
  ...
  reset(f1);
  while not eof(f1) do //запись чисел из фала в массив
  begin
    read(f1, f2[q]);
    inc(q);
    dline := dline + 1; //узнаем кол-во чисел в файле
  end;
  
  close(f1);
  q := 0;
  for q := 0 to dline do //сортировка массива по убыванию
    for p := q to dline do
    begin
      if f2[q] < f2[p] then
  ...
  for q := 0 to 4 do
  begin
    textout(320, qq, f2[q]);//вывод на экран таблицы с топ 5 рекордами
    qq := qq + 20;
  end;
  
  SetWindowSize(640, 480); //размер игрового окна
